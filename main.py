...
def main():
    ...
    def init_game(mode, difficulty):
        ...
        # Base dimensions from original assets (590x1004)
        aspect_ratio = 590 / 1004
        
        # Try to maximize height first
        target_h = (available_height - (rows + 1) * margin) // rows
        target_w = int(target_h * aspect_ratio)
        
        # If width is too wide, constraint by width instead
        if target_w * cols + (cols - 1) * margin > available_width:
            target_w = (available_width - (cols + 1) * margin) // cols
            target_h = int(target_w / aspect_ratio)
            
        # Optional: cap maximum size
        card_width = min(80, target_w)
        ...
        # STATE: PLAYING
                ...
                for col in range(board.cols):
                    ...
                    if img:
                        ...
                        if scaled_w > 0:
                            ...
                            scaled_img = create_rounded_surface(scaled_img, radius=15)
            ...
            def draw_captured(player, base_x, base_y):
                ...
                for i, symbol in enumerate(player.captured_symbols):
                    ...
                    scaled = create_rounded_surface(scaled, radius=8)
